      ...
      <div className="w-full max-w-4xl flex flex-col sm:border sm:border-base-300 sm:rounded-2xl overflow-hidden bg-base-100 shadow-xl">
        ...
        <div className="px-4 py-3 bg-base-100/95 backdrop-blur-sm border-b border-base-200 flex items-center justify-between sticky top-0 z-10">
          <div className="flex items-center gap-3">
            ...
            <div className="relative">
              <img
                ...
                alt=""
              />
              {loading ? "Connecting..." : onlineUsers.includes(otherUserId) ? (<div className="absolute bottom-0 right-0 w-3 h-3 bg-success rounded-full border-2 border-base-100"></div>) : (<div className="absolute bottom-0 right-0 w-3 h-3 bg-error rounded-full border-2 border-base-100"></div>)}
          ...
          <div className="flex items-center gap-2 sm:gap-4 text-base-content/70">
            ...
            <button onClick={() => startCall(false)} disabled={!onlineUsers.includes(otherUserId)} className="btn btn-ghost btn-circle btn-sm hover:text-primary transition-colors">
              <Phone className="w-5 h-5" />
            </button>
            <button onClick={() => startCall(true)} disabled={!onlineUsers.includes(otherUserId)} className="btn btn-ghost btn-circle btn-sm hover:text-primary transition-colors">
        ...
        <div className="p-3 bg-base-100 border-t border-base-200 flex items-end gap-2 shrink-0 z-10">
          ...
          <button
            onClick={sendMessage}
            className={`btn btn-circle shrink-0 transition-transform ${text.trim() ? "btn-primary hover:scale-105" : "bg-base-200 text-base-content/40 border-none pointer-events-none"
              }`}
